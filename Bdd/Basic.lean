      ...
      · ext x
        ...
        calc _
          _ = (O.high (by rw [O_def])).evaluate (x.set i true) := by
            ...
            apply Independence' (O.high (by rw [O_def])) ⟨i.1, hhi⟩
          ...
          _ = _ := by
            ...
            apply Independence' (O.low (by rw [O_def])) ⟨i.1, hhi⟩
    · exact hr'
  | tail r e =>
    rename_i p
...
lemma OBdd.usesVar_of_dependsOn {O : OBdd n m} {i : Fin n} :
  ...
  | node j =>
    ...
    | isFalse hf =>
      ...
      | inr hr =>
        have := (Independence' O ⟨i.1, by simp [var, Bdd.var, O_root_def]; omega⟩) b v
        contradiction
    | isTrue ht =>
      use j
      constructor
...
lemma OBdd.numPointers_node_le {O : OBdd n m} {h : O.1.root = node j} :
    O.numPointers ≤ 1 + (O.low h).numPointers + (O.high h).numPointers := by
  rw [numPointers_spec]
  rw [numPointers_spec]
  rw [numPointers_spec]
  ...
  · exact
    fun ⟨j', hj'⟩ ↦ match decEq j' j with
      | isTrue ht => .inl ⟨j', ht⟩
      | isFalse hf => .inr (
        match instDecidableReachable (O.low h) (node j') with
        | isTrue htt => .inl ⟨j', htt⟩
        | isFalse hff => .inr ⟨j', by
          apply reachable_or_eq_low_high at hj'
          cases hj' with
          | inl hh => simp_all
          | inr hh =>
            rcases hh with ⟨jj, hjj1, hjj2⟩
            have : node j = node jj := by rw [← h, hjj1]
  ...
  · intro x y hxy
    simp only [low_heap_eq_heap, high_heap_eq_heap] at hxy
    split at hxy
    next =>
...
lemma OBdd.numPointers_le' {O : OBdd n m} : O.numPointers ≤ 2 ^ (n - O.1.var.1) - 1 := by
  cases O_root_def : O.1.root with
  | terminal b => simp [isTerminal_iff_numPointer_eq_zero.mpr ⟨b, O_root_def⟩]
  | node j =>
    calc _
      _ ≤ 1 + (O.low O_root_def).numPointers + (O.high O_root_def).numPointers := numPointers_node_le
      _ ≤ 1 + (2 ^ (n - (O.low O_root_def).1.var.1) - 1) + (2 ^ (n - (O.high O_root_def).1.var.1) - 1) := by
        have := numPointers_le' (O := O.low O_root_def)
        have := numPointers_le' (O := O.high O_root_def)
        omega
      _ ≤ 1 + (2 ^ (n - (O.1.var.1 + 1)) - 1) + (2 ^ (n - (O.high O_root_def).1.var.1) - 1) := by
        simp only [Nat.succ_eq_add_one, low_heap_eq_heap, add_le_add_iff_right, add_le_add_iff_left, tsub_le_iff_right]
        rw [Nat.sub_add_cancel (by exact Nat.one_le_two_pow)]
        apply pow_le_pow_right' (by simp)
        have := var_lt_low_var (h := O_root_def)
        simp only [var] at this
        omega
      _ ≤ 1 + (2 ^ (n - (O.1.var.1 + 1)) - 1) + (2 ^ (n - (O.1.var.1 + 1)) - 1) := by
        simp only [Nat.succ_eq_add_one, low_heap_eq_heap, add_le_add_iff_right, add_le_add_iff_left, tsub_le_iff_right]
        rw [Nat.sub_add_cancel (by exact Nat.one_le_two_pow)]
        apply pow_le_pow_right' (by simp)
        have := var_lt_high_var (h := O_root_def)
        simp only [var] at this
        omega
      _ ≤ 1 + (2 ^ ((n - O.1.var.1) - 1) - 1) + (2 ^ (n - (O.1.var.1 + 1)) - 1) := by
...
lemma OBdd.numPointers_le {O : OBdd n m} : O.numPointers ≤ 2 ^ n - 1 := by
  trans 2 ^ (n - O.1.var.1) - 1
  · exact OBdd.numPointers_le'
